...
char* LinuxApplyStringStyle(char* str, enum LinuxStringColor color, enum LinuxStringStyle style, bool applyEndStyleTerminator)
{
	...
	if(color == None) 
	{
		memcpy(newStr, start, (strlen(start) - 1)*sizeof(char));
		memcpy(&newStr[strlen(start) - 1], str, strlen(str)*sizeof(char));
		memcpy(&newStr[strlen(start) - 1 + strlen(str)], reset, strlen(reset)*sizeof(char));
		newStr[strlen(start) - 1 + strlen(str) + strlen(reset)] = '\0';
	}else
	{
